...
class AlgorithmBase:
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            # 检查warmup是否完成（如果算法支持warmup）
            if hasattr(self, 'check_warmup_completion'):
                self.check_warmup_completion(epoch)
            ...
            for data_lb, data_ulb in zip(self.loader_dict['train_lb'],
                ...
                # 每轮训练后输出训练信息
                if self.it % self.num_log_iter == 0 or self.it == 1:
                    self.print_fn(f"Epoch {epoch + 1}, Iteration {self.it}: "
                                f"Total Loss: {self.log_dict.get('train/total_loss', 0.0):.4f}, "
                                f"Sup Loss: {self.log_dict.get('train/sup_loss', 0.0):.4f}, "
                                f"Unsup Loss: {self.log_dict.get('train/unsup_loss', 0.0):.4f}, "
                                f"Util Ratio: {self.log_dict.get('train/util_ratio', 0.0):.4f}")
